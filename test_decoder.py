...
para diferentes permutações (com novos objetivos: Custo e Insatisfação).
"""
...
context = parse_instance('evrptw_instances/rc108C5.txt')
...
# Permutação 3: aleatória 1
random.seed(42)
perm3 = list(range(n_customers))
random.shuffle(perm3)
permutations.append(("Aleatória 1", perm3))

# Permutação 4: aleatória 2
random.seed(123)
perm4 = list(range(n_customers))
random.shuffle(perm4)
permutations.append(("Aleatória 2", perm4))

# Permutação 5: primeira metade, depois segunda metade
mid = n_customers // 2
perm5 = list(range(mid, n_customers)) + list(range(mid))
permutations.append(("Segunda metade primeiro", perm5))

print("="*60)
print("TESTE: Decoder com Diferentes Permutações")
print("="*60)
...
for name, perm in permutations:
    ...
    results.append({
        'name': name,
        'perm': perm[:5],  # Primeiros 5 para mostrar
...
if len(set(f1_values)) == 1 and len(set(f2_values)) == 1:
    ...
    print("  Se n_nds ainda for 1, o problema pode estar no algoritmo (não mantém diversidade)")
